...
async def loop(self: BaseBot, user: User, message: str) -> None:
    ...
    async def loop_emote(self: BaseBot, user: User, emote_name: str) -> None:
        ...
        if user_position == None:
            await self.highrise.send_whisper(user.id,f"🚫🔄 @{user.username} pour stop le loop  il faut bouger🚫")
            return
        await self.highrise.send_whisper(user.id,f"👯🏻‍♂️🔄 @{user.username} Le loop est activé pour : {emote_name} 👯🏻‍♂️🔄")
